	...
	public:
		...
		Bitmap(size_t width, size_t height, unsigned int color = 0xFFFFFFFF){
			...
			for(size_t i(0); i < pixel_n; ++i){
				...
				mapdata[i].alpha	= (BYTE)(color & 0x000000FF);
				mapdata[i].blue		= (BYTE)((color & 0x0000FF00) >> 8);
				mapdata[i].green	= (BYTE)((color & 0x00FF0000) >> 16);
				mapdata[i].red		= (BYTE)((color & 0xFF000000) >> 24);
			}
		}
		~Bitmap(){
			delete[] mapdata;
